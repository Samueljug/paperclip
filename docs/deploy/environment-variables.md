...
| `FACTORY_RUNS_DIR` | (unset) | Path to the factory runs directory. If both `DARK_FACTORY_RUN_DIR` and `FACTORY_RUNS_DIR` are unset, defaults to `../paperclip-data/factory-runs` relative to the server source root. |
