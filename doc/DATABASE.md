...
Environment env variables live in `environments.env_vars` (or `environments.config.envVars`). Environment secret refs bind with `target_type = 'environment'`, `target_id = environments.id`, and `config_path` values under `env_vars.*`.
